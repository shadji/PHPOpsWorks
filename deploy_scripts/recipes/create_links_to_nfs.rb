...
node[:deploy].each do |application, deploy|
	...
	node[:sites].each do |site|
		...
		Chef::Log.info("Createing symlink: sudo -H -u #{user} bash -c 'cd #{deploy["php"]["symlinks"]} && ln -s #{site.link} ./#{site.name}/#{site.directory}")
		Chef::Log.info(`sudo -H -u #{user} bash -c 'cd #{deploy[:deploy_to]} && ln -s #{site.link} ./#{site.name}/#{site.directory}'`)
